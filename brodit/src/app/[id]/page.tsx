...
export default function BroditView() {
  const { id } = useParams()
  ...
  const [rawBrodit, setRawBrodit] = useState<RawBrodit>()

  const loadBrodit = async () => {
    const contract = getContract()
    const brodit = await contract.getBrodit(id)

    const expirationDate = new Date(brodit.expirationDate.toNumber())
    const item: Item = { ...brodit, expirationDate }
  ...
  const unseal = async () => {
    ...
    try {
      const brodit = await pullBrodit(item.broditCid, password)

      setStep(3)
      setRawBrodit(brodit)
      console.log('Ready', brodit)
    } catch {
      setStep(1)
  ...
  const downloadFile = () => {
    if (!rawBrodit || rawBrodit.files.length == 0) return
    const file = rawBrodit.files[0]
  ...
  return (
    <div className="w-full px-[20px] h-[100vh] mx-auto flex flex-col items-center justify-center">
      ...
      <h2 className="text-3xl text-center font-light mt-[12px]">
        {step === 0 && 'Here is Your Brodit.'}
        {step === 1 && 'Enter the password to unseal the Brodit.'}
        {step === 2 && 'Unsealing the Brodit...'}
      </h2>
      {(step === 0 || step === 1) && (
        ...
        <div className="flex-1 w-full max-w-7xl">
          ...
          <div className="flex gap-4 mt-5">
            <div className="border-border border rounded-[20px] px-10 py-6 flex-1">
              ...
              <div className="text-xl font-light mt-3">{rawBrodit?.description}</div>
            </div>
            <div
